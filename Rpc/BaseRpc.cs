...
namespace PokemonGo.RocketAPI.Rpc
{
    public class BaseRpc
    {
        ...
        protected RequestBuilder RequestBuilder => new RequestBuilder(_client.AuthToken, _client.AuthType, _client.CurrentLatitude, _client.CurrentLongitude, _client.CurrentAltitude, 5.0,  _client.Settings, _client.AuthTicket);
        ...
        protected async Task<Tuple<T1, T2, T3, T4, T5, T6>> PostProtoPayload<TRequest, T1, T2, T3, T4, T5, T6>(RequestEnvelope requestEnvelope) where TRequest : IMessage<TRequest>
            where T1 : class, IMessage<T1>, new()
            where T2 : class, IMessage<T2>, new()
            where T3 : class, IMessage<T3>, new()
            where T4 : class, IMessage<T4>, new()
        ...
        {
            var responses = await PostProtoPayload<TRequest>(requestEnvelope, typeof(T1), typeof(T2), typeof(T3), typeof(T4), typeof(T5), typeof(T6));
            return new Tuple<T1, T2, T3, T4, T5, T6>(responses[0] as T1, responses[1] as T2, responses[2] as T3, responses[3] as T4, responses[3] as T5, responses[3] as T6);
        }
